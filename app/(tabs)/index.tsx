...
import { Plus, Minus, Fuel, Wallet, TrendingUp, Droplet, Banknote, CarFront, HandCoins } from 'lucide-react-native';
    ...
    <SafeAreaView style={{ flex: 1 }} edges={['top', 'bottom']}>
      <Box className="flex-1 bg-background-50 dark:bg-background-0" style={{ marginBottom: -48 }}>
                    ...
                    <Box className="bg-black/70 p-6" style={{ minHeight: 200, position: 'relative' }}>
                      ...
                      {selectedVehicle.vehicle_type_name_key && (
                        <Box style={{ position: 'absolute', top: 16, right: 16 }}>
                          <Box className="bg-white dark:bg-black px-3 py-1.5 rounded-lg">
                            <Text className="text-xs font-semibold text-typography-900 dark:text-white">
                        ...
                        <VStack space="xs">
                          ...
                          {selectedVehicle.license_plate && (
                            <HStack className="items-center mt-2">
                              <Box className="bg-white dark:bg-black px-3 py-1.5 rounded-lg">
                                <Text className="text-base font-bold text-typography-900 dark:text-white tracking-wider">
                                  {selectedVehicle.license_plate}
                                </Text>
                              </Box>
                            </HStack>
            ...
            <Card className="p-5 shadow-lg bg-white dark:bg-background-50">
              <VStack space="lg">
                <HStack className="items-center" space="sm">
                  <Icon as={Fuel} className="w-6 h-6 text-orange-500" />
                ...
                <VStack space="md">
                  ...
                  <Input variant="outline" className="border-2 border-background-200 dark:border-background-700 rounded-xl">
                    <InputField
                      ...
                      value={fuelUnitPrice}
                      onChangeText={(text) => setFuelUnitPrice(text.replace(/[^0-9.,]/g, ''))}
                      keyboardType="numeric"
                      className="text-base"
                    />
                  </Input>
                  ...
                  <HStack space="sm" alignItems="center">
                    ...
                    >
                      <ButtonIcon as={Plus} size={12} className="text-typography-900" />
                    </Button>
                    <Button
                      size="lg"
                      variant="outline"
                      ...
                      className="rounded-full h-6 w-6 p-0 flex-none border-background-200 dark:border-background-700"
                    >
                      <ButtonIcon as={Minus} size={12} className="text-typography-900" />
                    </Button>
                  </HStack>
              ...
              {transactions.length === 0 ? (
                ...
                <VStack space="sm">
                  {transactions.map((transaction, index) => {
                    ...
                    const isFuel = !isIncome && (
                      transaction.expense_type_name?.toLowerCase().includes('fuel') ||
                      transaction.expense_type_name?.toLowerCase().includes('yakıt')
                    );

                    let borderClass = 'border-l-error-500';
                    if (isIncome) borderClass = 'border-l-success-500';
                    else if (isFuel) borderClass = 'border-l-orange-500';

                    let boxBgColor = 'rgba(239, 68, 68, 0.1)';
                    if (isIncome) boxBgColor = 'rgba(34, 197, 94, 0.1)';
                    else if (isFuel) boxBgColor = 'rgba(249, 115, 22, 0.1)';

                    return (
                      <Card key={transaction.id} className={`p-4 shadow-md bg-white dark:bg-background-50 border-l-4 ${borderClass}`}>
                        <VStack space="sm">
                            ...
                            <HStack space="sm" className="flex-1 items-center">
                              ...
                              >
                                {(() => {
                                  ...
                                  return <Icon as={iconName} size="xl" className={iconClass} />;
                                })()}
                              </Box>
                              <VStack space="xs" className="flex-1">
                                ...
                                {transaction.fuel_unit_price && (
                                  <HStack space="xs" className="items-center">
                                    <Icon as={HandCoins} className="w-[18px] h-[18px] text-typography-400" />
                            ...
                            <VStack className="items-end" space="xs">
                              <Text
                                className={`font-bold text-lg ${(() => {
                                  if (isIncome) return 'text-success-600';
                                  if (isFuel) return 'text-orange-500';
                          ...
                          {/* Fuel Metrics */}
                          {fuelMetrics && (
                            <Box className="bg-background-50 dark:bg-black/20 p-3 rounded-lg mt-2 border border-background-200 dark:border-background-800">
                              <HStack space="lg" className="justify-around">
                                <VStack space="xs" className="items-center">
                                  <HStack space="xs" className="items-center">
                                    <Icon as={CarFront} className="w-[18px] h-[18px] text-typography-500 dark:text-typography-400" />
                                    <Text className="text-xs text-typography-500 dark:text-typography-400 uppercase">{t('fuel.distance')}</Text>
                                  </HStack>
                                ...
                                <VStack space="xs" className="items-center">
                                  <HStack space="xs" className="items-center">
                                    <Icon as={Banknote} className="w-[18px] h-[18px] text-typography-500 dark:text-typography-400" />
                                    <Text className="text-xs text-typography-500 dark:text-typography-400 uppercase">{t('fuel.costPerKm')}</Text>
                                  </HStack>
                                  <Text className="font-bold text-sm text-error-600 dark:text-error-400">
                                ...
                                {fuelMetrics.litersPer100km && (
                                  <VStack space="xs" className="items-center">
                                    <HStack space="xs" className="items-center">
                                      <Icon as={Droplet} className="w-[18px] h-[18px] text-typography-500 dark:text-typography-400" />
                                      <Text className="text-xs text-typography-500 dark:text-typography-400 uppercase">{t('fuel.consumption')}</Text>
                                    </HStack>
                                    <Text className="font-bold text-sm text-warning-600 dark:text-warning-400">
              ...
              <VStack space="md">
                ...
                {editCurrency && editCurrency !== defaultCurrency && (
                  <Input variant="outline" className="border-2 border-error-300 bg-error-50">
                    <InputField
                      ...
                      onChangeText={(text) => setEditExchangeRate(text.replace(/[^0-9.,]/g, ''))}
                      keyboardType="numeric"
                    />
                  </Input>
                )}
                ...
                <Input variant="outline">
                  <InputField
                    ...
                    onChangeText={(text) => setEditFuelUnitPrice(text.replace(/[^0-9.,]/g, ''))}
                    keyboardType="numeric"
                  />
                </Input>
                ...
                <Input variant="outline">
                  <InputField
                    ...
                    value={editOdometer}
                    onChangeText={(text) => setEditOdometer(text.replace(/[^0-9]/g, ''))}
                    keyboardType="numeric"
                  />
                </Input>
        ...
        <Fab
          ...
          onPress={() => router.push('/drawer')}
        >
          <FabIcon as={CarFront} className="w-8 h-8" />
        </Fab>
      </Box>
    </SafeAreaView>
  );
